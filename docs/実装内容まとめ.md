...
| 項目 | パス・内容 |
|------|------------|
| 実装 | `scripts/lib/jraDriver.mjs`（ルートからは `lib/jraDriver.mjs` が再エクスポート） |
| 役割 | Playwright で **単勝オッズ・枠・人気** を取得。**`fetchJraOddsAndFramesKeyedOrNull`** で馬番キー・失敗時 `null`。 |

詳細は `docs/scraping-architecture.md`。
