...
| **オッズ／買い目** | **`final_expected_value` 基準の表示** + **`generateBetTicketsFromEvaluation`（Harville EV推奨券）**、`betBuilder`、ヒートマップ |
...
購入・バックテストの正は `buildRaceBettingContext` → `evTickets`。  
生成ロジックは `src/domain/betting/bettingRules.ts` の `generateTicketsFromEvaluation`。

| 項目 | 内容 |
|------|------|
| 方式 | 勝率 ≥ 1% の馬で Harville 合成確率 × オッズ。**単勝は ◎のみ**（最大1点）。馬連・ワイドは全2頭組み合わせ。**3連複は TS=全3頭組み合わせ、AI=◎×2列目3×3列目8** |
| 閾値 | WIN=1.3 / REN=1.80 / WIDE=1.30 / TRI=2.00（固定・`investmentEvConstants.ts`） |
| 上限 | WIN・REN 各10点 / WIDE 5点 / TRI 3点（EV降順） |
| TS/AI | 馬連・ワイド・単勝は同一。**3連複のみ AI で超大穴フィルター**（100倍上限2頭・150倍無印除外） |

詳細: [実装ロジック詳細一覧.md §6.2](./実装ロジック詳細一覧.md)、[馬券的中ロジックまとめ.md](./馬券的中ロジックまとめ.md)

---

## 9. オッズ・Python AI（2026-05 現状）
...
| 買い目 UI | `src/components/race/RaceBetPanel.tsx`, `betBuilder.ts` |
| EV推奨券 | `src/domain/betting/bettingRules.ts`, `buildRaceBettingContext.ts` |
