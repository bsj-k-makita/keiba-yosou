...
| 買い目 EV | `src/domain/betting/bettingRules.ts` — **Harville合成確率×オッズの純粋EV**（TS/AI共通）。閾値は `investmentEvConstants.ts` |
...
| ◎ | **ai_effective_ev 最上位**（勝率フロアなし・期待値優先） |
...
| 買い目 | `generateBetTicketsFromEvaluation`（Harville EV）+ enrich の `final_expected_value`（表示列） |
...
| 日付 | 内容 |
|------|------|
| 2026-05-23 | EV推奨券を純粋EVベースに変更（Harville×固定閾値）。◎軸フォーメーション撤廃 |
| 2026-05-24 | EV閾値引き上げ（REN=1.80/WIDE=1.30/TRI=2.00）、AI 3連複列ベース＋NO_EV異常検知拡張 |
