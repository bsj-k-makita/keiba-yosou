...
1. **確定払戻** — 完了。
2. **クラス判定厳密化** — 完了。`resolveEffectiveRaceClass.ts` + `raceGrade` を `RaceCondition` へ。
3. **3連複フォーメ改造** — 完了（◎ × 拡張2列目 × △☆、爆穴☆2列目昇格）。

## クラス階層バックテスト（70レース・確定払戻）
...
### 3連複フォーメ改造後（70レース・確定払戻）

| 券種 | 回収率 | 備考 |
|------|--------|------|
| 単勝◎ | 51.9% | 券種ロジック不変 |
| 馬連◎○▲ | 101.8% | 券種ロジック不変 |
| 3連複（1-3-N型） | 4.6% | 投資 60,100円 |
| **合計** | **31.5%** | 前回 25.2% から +6.3pt |
