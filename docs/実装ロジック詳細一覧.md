...
（2026年5月時点の `src/`・`scripts/`・`python/` を対象。2026-05-20 に EV推奨券一本化・UI簡素化を反映）
...
| レース詳細・結果 | タブ `result` | 着順・**EV券**払戻内訳・見送り時「投資0/払戻0/回収0%」 |
...
| **Python AI / TS 切替** | 詳細ヘッダー | 既定 **Python AI**。`?engine=ts` で TS 明示 |
| **方針B: AI印** | AIモード時 | `ai_effective_ev` 降順で ◎○▲☆△△△（7頭）。TS印は破棄 |
| **NO_EV 見送りレジーム** | AIモード時 | 全頭EV横並び → 警告バナー＋相対印表示・**EV推奨0点**（§5.4） |
| レースナビ | 詳細上部 | 日付/場/R番号タブ、前後R移動 |
| netkeiba リンク | 詳細ヘッダー | 出馬表・結果ページへのリンク |
| 回収率バックテスト画面 | `/backtest` | **EV推奨券**集計サマリ + TS/AI比較 + 結果確定全R一覧（的中/見送り/不的中カード） |
...
| 直前オッズ | `fetch-live-odds` | `market_win_odds` / `actual` |
...
| 案A | 全頭EVが横並びのレースは **相対印は付与**・**EV推奨0点**＋警告バナー |
...
- **Phase1 Python ML（`entries[]` 任意）**:
  ...
  - `ai_effective_ev` — `(ai_predicted_win_rate × 単勝オッズ) − 0.15`
...
| `/race/:raceId` | `RaceDetailPage` | `getRaceEvaluationById`、dev時4秒ポーリング、**既定AI**・`?engine=ts` |
| `/backtest` | `BacktestDashboardPage` | EV集計サマリ + TS/AI比較 + 結果確定全R一覧 |

### 3.3 勝率エンジン切替（Phase 1 → 方針B → 案A）

レース詳細（`RaceDetailView`）で **Python AI**（既定） / **TS評価** を切替。初期タブは **出馬表**。URL 未指定＝AI、`?engine=ts` で TS。
...
| **TS** | Softmax(score, T=1.5) | `final_expected_value` | 1.3 | `ensureFrontendDisplayMarks` | EV推奨券（TS閾値） | TS再計算 |
...
NO_EV_REGIME ⇔ maxEv < EV_WORTHLESS_THRESHOLD (−0.10)
            AND stdev < EV_STDEV_THRESHOLD (0.01)
```
...
**検証レース例:** `202605021011`（オークス）— 全頭 `ai_effective_ev = −0.15`、stdev≈0 → NO_EV。
...
- **AI単勝:** `aiEffectiveEv >= AI_EFFECTIVE_EV_THRESHOLD`。**単勝オッズ欠損でもEV判定可**（2026-05-20）
- **馬連・ワイド・3連複:** 組み合わせに **◎（topMarkGate）を必須含む**（`pairIncludesTopMark`）
- 券種ごと **EV降順キャップ**（`capEvTicketsByType`）: 単勝・馬連・ワイド各10点、3連複5点

**閾値（`investmentEvConstants.ts`）:**

| 券種 | TS | AI |
|------|-----|-----|
| 単勝（WIN） | 1.3 | **1.30**（`ai_effective_ev` 直判定） |
| 馬連（REN） | 1.3 | **1.60** − margin(0.15) |
| ワイド（WREN） | 1.3 | **1.20** − margin |
| 3連複（TRI） | 1.5 | **1.60** − margin |
| MAIDEN_NEW | — | 3連複ループ自体スキップ |
...
| コンポーネント | 責務 |
|----------------|------|
| `RaceDetailView` | パイプライン実行、AI/TSトグル、NO_EV警告バナー、タブ制御 |
| `RaceNavBar` | 日付/場/R番号、前後R |
| `RaceAdjustmentPanel` | 条件スライダー（即時再評価） |
| `RaceHorsesView` | 出馬表（NO_EV でも印表示） |
| `HorseListTable` | 印・着順・EV列。AI/TSラベル |
| `HorseEvaluationCard` | 能力レーダー・スコア内訳 |
| `RaceBettingDashboard` | **EV推奨券のみ**。0点時は見送り1行表示 |
| `RaceConclusionPanel` | 結論テキスト（◎○▲） |
| `RaceResultAnalysis` | EV券払戻表。`probabilityEngine` をパイプラインから受け取る |
| `RaceListCard` | outcome: 的中 / 見送り / 不的中 |
| `EvHeatmap` | 馬×券種EVヒートマップ |
| `NetkeibaRaceLinks` | 外部リンク |

**CSS（`index.css`）:** `.ai-no-ev-banner`, `.bt-hit-card--skip`, `.result-analysis-view__ev-skip`, `.rl-page__legend`

### 8.2 一覧・バックテスト

| コンポーネント | 責務 |
|----------------|------|
| `RacesListPage` | 日付タブ、30R統計、一括結果取得 |
| `RaceListCard` | プレビュー・outcome色（的中/不的中） |
| `BacktestDashboardPage` | サマリ・比較・ヒット一覧のレイアウト |
...
| AI_EFFECTIVE_EV_THRESHOLD | **1.30** | `investmentEvConstants.ts`（AI単勝） |
| REN_EV_THRESHOLD | **1.60** | `investmentEvConstants.ts`（AI馬連） |
| WIDE_EV_THRESHOLD | **1.20** | `investmentEvConstants.ts`（AIワイド） |
| TRI_EV_THRESHOLD | **1.60** | `investmentEvConstants.ts`（AI3連複） |
| EV_MAX_TICKETS_PER_TYPE | 10 | 単勝・馬連・ワイド上限 |
| EV_MAX_TRI_TICKETS_PER_TYPE | 5 | 3連複上限 |
