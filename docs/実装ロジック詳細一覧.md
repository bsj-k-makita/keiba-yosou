...
| `/backtest` | `BacktestDashboardPage` | `backtest_summary.json`（**Python AI 印**）+ `backtest_comparison.json`（TS参考） |

### 3.3 勝率エンジン切替（Phase 1 → 方針B 完全連動）

レース詳細（`RaceDetailView`）で **Python AI**（既定） / **TS評価** を切替。初期タブは **出馬表**。URL クエリ `?engine=ts` で TS 明示。

| モード | 勝率 | 期待値表示 | EV推奨閾値 | 印・定型買い目 | スコア・点数 |
|--------|------|------------|------------|----------------|--------------|
| **AI**（既定・方針B） | JSON `ai_predicted_win_rate` | JSON `ai_effective_ev`（カード最優先） | 1.05（`AI_EFFECTIVE_EV_THRESHOLD`） | **`ai_effective_ev` 降順で ◎○▲☆△ を再割当** → フォーメ買い目に連動 | TS 表示のまま（参考） |
| **TS**（`?engine=ts`） | Softmax(`finalEvaluationScore`, T=1.5) | JSON `final_expected_value` | 1.3（`DEFAULT_EV_THRESHOLD`） | `evaluateRace` + `ensureFrontendDisplayMarks` | TS 再計算 |

実装:
- `src/lib/pipeline/probabilityEngine.ts` — 勝率 Map 解決・フルバックフィル判定（`raceHasFullAiBackfill`）
- `src/lib/pipeline/aiMarkAssignment.ts` — 方針B 印の機械再割当（`applyAiMarksByEffectiveEv`）
- `runRaceEvaluationPipeline({ probabilityEngine: 'ai' })` — 印上書き後に ViewModel・馬券生成

**フォールバック:** 1頭でも `ai_predicted_win_rate` / `ai_effective_ev` が欠損 → TS にフォールバック（クラッシュ防止）。TS評価ボタンは引き続き利用可。

**未連携:** レース一覧の直近30R回収率は TS バックテストのまま。`/backtest` は Python AI（`npm run backtest:bets`）。
...
### 10.9 TS との意図的アライメント（方針B 完全連動）

| 概念 | Python | TypeScript | 連携状況 |
|------|--------|------------|----------|
| 確率フロア | 0.01 | `VALID_PROB_THRESHOLD` | 未統一 |
| EV 閾値 | 1.05（`MIN_EV_THRESHOLD` / simulate） | TS: 1.3 / **AI: 1.05** | **AI モードで統一** |
| 実質 EV 式 | `P×O - 0.15` | `ai_effective_ev`（バックフィル） | AI モードで表示・単勝 EV 券に使用 |
| 券種 | 単勝のみ（simulate） | WIN/REN/WREN/TRI | 別系統（AI でも複勝系は TS 確率でペア推定） |
| 確率源 | LGBM + Isotonic + レース内正規化 | AI: `ai_predicted_win_rate` / TS: Softmax | レース詳細 `?engine=ai` |
| 印・買い目 | — | **AI: `ai_effective_ev` 降順で ◎○▲☆△ 再割当** | **方針B（2026-05）** — フォーメ◎軸も AI 最上位 |
| スコア表示 | — | `evaluateRace` | **常に TS 参考表示**（条件スライダー連動） |
...
2. **バックフィル:** `scripts/backfill-ai-predictions.py` → `probabilityEngine.ts` + `aiMarkAssignment.ts` → `RaceDetailView` トグル（§3.3 方針B）
3. **テスト:** `probabilityEngine.test.ts` / `aiMarkAssignment.test.ts` / `evaluationPipeline.ai.test.ts`（フルバックフィル・印再割当・フォールバック）
