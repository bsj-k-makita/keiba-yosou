...
- **TS / AI 共通**の純粋EVロジック（フォーメーション分岐・◎軸縛りなし）

**組み合わせ生成（2026-05-23 以降）:**

1. `alignHorsesWithProbabilities` で有効馬リストを取得
2. **単勝:** `topMarkGate`（◎）のみ EV 判定（最大1点）
3. 有効馬から **全2頭組み合わせ**（馬連・ワイド）・**全3頭組み合わせ**（3連複）を列挙
3. 各組み合わせについて Harville 合成確率を計算
4. `buildOddsMapForEvEvaluation` で単勝から推定した複勝券オッズをマージ（実オッズ優先）
5. `EV = 合成確率 × オッズ` を算出し、券種別固定閾値以上のみ採用
6. EV降順で `capEvTicketsByType` により券種別上限で切り捨て
...
| REN | `REN_EV_THRESHOLD` | 1.60 |
| WREN | `WIDE_EV_THRESHOLD` | 1.20 |
| TRI | `TRI_EV_THRESHOLD` | 1.50 |

**券種別上限（EV降順）:** WIN/REN=10、WREN=5、TRI=3（`capEvTicketsByType`）

**撤廃済み（2026-05-23）:**

- `createFixedAiFormationTickets`（AI ◎軸フォーメーション）
- 複勝券の **◎（topMarkGate）必須含有**
- ワイド相手馬の単勝オッズ下限（`WIDE_PARTNER_MIN_WIN_ODDS`）
- 動的EV閾値（`resolveDynamicEvThreshold`）
- AI専用の脚質分散2列目・印付き相手優先・3連複全滅回避1点
...
| REN_EV_THRESHOLD | **1.60** | `investmentEvConstants.ts` |
| WIDE_EV_THRESHOLD | **1.20** | `investmentEvConstants.ts` |
| TRI_EV_THRESHOLD | **1.50** | `investmentEvConstants.ts` |
...
- EV推奨券生成を **純粋EVベース** にリファクタリング（`generateTicketsFromEvaluation`）
  - ◎軸フォーメーション（`createFixedAiFormationTickets`）・AI/TS分岐を撤廃
  - 勝率 ≥ 1% の全馬から馬連/ワイド/3連複の **全組み合わせ** を評価
  - Harville 合成確率（3連複は全6順列合算）× オッズで EV 算出
  - 複勝券オッズ欠損時は `buildOddsMapForEvEvaluation` の推定オッズを使用
  - 固定閾値（REN=1.60 / WIDE=1.20 / TRI=1.50）＋券種別EV降順キャップ（10/10/5/3）
