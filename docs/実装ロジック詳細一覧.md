...
| 機能 | モジュール | 要点 |
|------|------------|------|
| **EV推奨券（正）** | `generateBetTicketsFromEvaluation` | UI・BT・一覧の**唯一の購入ソース**。Harville合成確率×オッズの**純粋EV**＋券種別閾値＋点数キャップ |
...
NO_EV_REGIME ⇔ (1番人気相当・勝率1%以下の異常)
            OR (stdevP < WIN_RATE_STDEV_THRESHOLD (0.005))
            OR (stdevP < WIN_RATE_STDEV_NEAR_FLAT_THRESHOLD (0.01)
                AND maxEv < EV_WORTHLESS_THRESHOLD (−0.10))
...
| `WIN_RATE_STDEV_NEAR_FLAT_THRESHOLD` | 0.01 | 0.005すり抜け時の二次検知（maxEv低いとき） |
| `FAVORITE_ANOMALY_MAX_WIN_ODDS` | 10.0 | この倍率以下で勝率≤1%なら過収縮異常 |
| `ANOMALY_WIN_RATE_FLOOR` | 0.01 | 異常検知用の勝率床（1%） |
...
**検証:** 勝率横並び（`stdevP < 0.005`）の合成データで NO_EV を確認。実レース `202605021011` は再学習後に分布が変わるため、回帰テストは合成ケースを使用。
...
- `topMarkGate`（◎の枠番）は **UI・診断用**（8%ルール等）。**EV券生成の軸条件には使わない**
...
### 6.2 EV推奨券生成（`generateTicketsFromEvaluation` / `generateBetTicketsFromEvaluation`）

**入口:** `generateBetTicketsFromEvaluation` → `buildRaceEvaluationContext` + `generateTicketsFromEvaluation` → `evTicketsToBetTickets`

**実装:** `src/domain/betting/bettingRules.ts`

**前提:**

- `isSkippableRace === true`（**TSモードのみ**）→ EV券ゼロ
- 勝率 < 1% の馬は全券種から除外（`VALID_PROB_THRESHOLD = 0.01`）
- 馬連・ワイド・単勝は **TS / AI 共通**の純粋EV。**3連複のみ AI で列ベース生成**

**組み合わせ生成（2026-05-24 現行）:**

1. `alignHorsesWithProbabilities` で有効馬リストを取得
2. **単勝:** `topMarkGate`（◎）のみ EV 判定（最大1点）
3. 有効馬から **全2頭組み合わせ**（馬連・ワイド）
4. **3連複:** TS は **全3頭組み合わせ** / AI は **`◎ × 2列目上位3 × 3列目上位8`**（`buildAiTrifectaThirdColumnGates`）
5. 各組み合わせについて Harville 合成確率を計算
6. `buildOddsMapForEvEvaluation` で単勝から推定した複勝券オッズをマージ（実オッズ優先）
7. `EV = 合成確率 × オッズ` を算出し、券種別固定閾値以上のみ採用
8. EV降順で `capEvTicketsByType` により券種別上限で切り捨て

**券種別閾値（固定）:**

| 券種 | 閾値定数 | 値 |
|------|----------|-----|
| WIN | `DEFAULT_EV_THRESHOLD` / `thresholdEV` | 1.3 |
| REN | `REN_EV_THRESHOLD` | 1.80 |
| WREN | `WIDE_EV_THRESHOLD` | 1.30 |
| TRI | `TRI_EV_THRESHOLD` | 2.00 |

**AI 3連複・3列目制限（`investmentEvConstants.ts`）:**

| 定数 | 値 | 意味 |
|------|-----|------|
| `AI_TRI_THIRD_COLUMN_SIZE` | 8 | 3列目候補枠 |
| `AI_TRI_SECOND_COLUMN_SIZE` | 3 | 2列目候補枠 |
| `AI_TRI_PREFERRED_ODDS_MIN` / `MAX` | 10 / 80 | 3列目で優先する単勝帯 |
| `AI_TRI_ULTRA_LONGSHOT_ODDS_CAP` | 100 | 3列目の100倍以上は最大2頭 |
| `AI_TRI_ULTRA_LONGSHOT_MAX_COUNT` | 2 | 上記の頭数上限 |
| `AI_TRI_NOISE_EXCLUSION_ODDS` | 150 | 無印150倍以上は3列目から除外 |

**券種別上限（EV降順）:** WIN/REN=10、WREN=5、TRI=3（`capEvTicketsByType`）

**撤廃済み（2026-05-23）:**

- `createFixedAiFormationTickets`（旧 AI ◎軸フォーメーション一式）
- 複勝券の **◎（topMarkGate）必須含有**
- 動的EV閾値（`resolveDynamicEvThreshold`）
- 3連複0点時の全滅回避1点

**オッズマップ（`buildOddsMapForEvEvaluation`）:**

- 単勝: `market_win_odds` → `evaluationSignals.winOdds`（`enrichEntryEvaluationSignals`）
- 複勝券: 直前/確定オッズが無い場合は単勝・勝率から **Harville型で推定**（`appendEstimatedExoticOdds`）
...
3連複（全6通りの順列を合算）:
  ...
  estimateTrifectaProbability は上記を [A,B,C] の全順列について合算

EV = 合成確率 × オッズ（馬連・ワイド・単勝は TS/AI 共通。3連複は AI のみ列ベース）
```

**Kelly配分:** `evTicketsToBetTickets` で券種別ケリー係数（WIN/REN/WREN=0.25、TRI=0.15）を適用。1レース総投資は `MAX_INVESTMENT_PER_RACE = 8,000`（`BETTING_MAX_INVESTMENT_PER_RACE` で上書き可）。
...
- **期待値シグモイド重み付け**（`ENABLE_EV_SAMPLE_WEIGHT=True` 時）:
  ...
  2. 理論 EV = `oof_pred × odds`（オッズ欠損・0以下は raw_weight=1.0）
  3. 動的シグモイド `raw = 1 + sigmoid((EV-center)/tau)` → レース内で **合計 1** に再正規化
     - 通常: `center=0.95`, `tau=0.05`
     - 重賞（`graded_race_tier >= 6` = G3/G2/G1）: `center=1.05`, `tau=0.10`（大穴過学習抑制）
...
| 購入集計 | simulate単勝 | **`evTickets` のみ** | 全組み合わせ Harville EV（◎軸なし） |
| 印 | — | EV降順◎○▲☆△△△ | **方針B** |
...
| DEFAULT_EV_THRESHOLD | 1.3 | `bettingRules.ts`（単勝） |
| AI_EFFECTIVE_EV_THRESHOLD | **1.30** | `investmentEvConstants.ts`（UI表示しきい値文言） |
| REN_EV_THRESHOLD | **1.80** | `investmentEvConstants.ts` |
| WIDE_EV_THRESHOLD | **1.30** | `investmentEvConstants.ts` |
| TRI_EV_THRESHOLD | **2.00** | `investmentEvConstants.ts` |
| AI_TRI_THIRD_COLUMN_SIZE | 8 | `investmentEvConstants.ts`（AI 3列目枠） |
| AI_TRI_SECOND_COLUMN_SIZE | 3 | `investmentEvConstants.ts`（AI 2列目枠） |
| AI_TRI_PREFERRED_ODDS_MIN / MAX | 10 / 80 | `investmentEvConstants.ts`（3列目優先帯） |
| AI_TRI_ULTRA_LONGSHOT_ODDS_CAP | 100 | `investmentEvConstants.ts`（3列目100倍制限） |
| AI_TRI_ULTRA_LONGSHOT_MAX_COUNT | 2 | 同上 |
| AI_TRI_NOISE_EXCLUSION_ODDS | 150 | 同上（無印除外） |
...
| EV_MAX_TRI_TICKETS_PER_TYPE | 3 | `investmentEvConstants.ts`（TRI上限） |
| ANCHOR_MIN_PREDICTED_WIN_RATE | 0.08 | `investmentEvConstants.ts`（◎表示・診断用。EV券軸条件には不使用） |
...
| WIN_RATE_STDEV_NEAR_FLAT_THRESHOLD | 0.01 | `aiEvRegime.ts`（すり抜け二次検知） |
| FAVORITE_ANOMALY_MAX_WIN_ODDS | 10.0 | `aiEvRegime.ts` |
| ANOMALY_WIN_RATE_FLOOR | 0.01 | `aiEvRegime.ts` |
...
| EV_WEIGHT_CENTER | 0.95 | `config.py`（通常レース・シグモイド中心） |
| EV_WEIGHT_TAU | 0.05 | `config.py`（通常レース・シグモイド温度） |
| EV_WEIGHT_CENTER_GRADED | 1.05 | `config.py`（G3/G2/G1向け中心・厳格化） |
| EV_WEIGHT_TAU_GRADED | 0.10 | `config.py`（G3/G2/G1向け温度・なだらか化） |
| EV_WEIGHT_GRADED_MIN_TIER | 6.0 | `config.py`（graded_race_tier 閾値: G3=6） |
| EV_WEIGHT_GRADED_CLASSES | ("G1","G2","G3") | `config.py`（tier未計算時のフォールバック） |
| EV_WEIGHT_ODDS_CAP | 100.0 | `config.py`（学習重み内部EVの上限） |

補足（2026-05）:
- 重賞は市場オッズの歪みが小さいため、center を **引き上げ**（1.05）、tau を **広げ**（0.10）て境界付近の大穴への学習重み急昇を抑制する（旧: center=0.75 で逆に大穴を強化していた）。
- `graded_race_tier >= 6`（G3/G2/G1）で graded パラメータを適用。tier 未計算データは `race_class in ("G1","G2","G3")` にフォールバック。
...
| `domain/betting/*.test.ts` | EV計算、Harville、全組み合わせEV、払戻、見送り |
...
- EV閾値引き上げ: **REN=1.80 / WIDE=1.30 / TRI=2.00**（`investmentEvConstants.ts`）
- AI 3連複: **◎×2列目3×3列目8** の列ベース生成＋超大穴フィルター（100倍最大2頭・150倍無印除外）
- `resolveAiRaceRegime`: 1番人気の勝率崩壊検知、勝率stdev 0.01 二次検知を追加

**主な変更点（2026-05-23）:**

- EV推奨券生成を **純粋EVベース** にリファクタリング（`generateTicketsFromEvaluation`）
  - 旧 `createFixedAiFormationTickets` 撤廃
  - 馬連/ワイドは全組み合わせ × Harville × オッズ
  - 複勝券オッズ欠損時は `buildOddsMapForEvEvaluation` の推定オッズを使用
  - 券種別EV降順キャップ（10/10/5/3）
...
- AI EV閾値（単勝1.30、馬連1.80、ワイド1.30、3連複2.00）、券種別点数キャップ
